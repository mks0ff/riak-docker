...
RUN sed -i.bak 's/listener.http.internal = 127.0.0.1/listener.http.internal = 0.0.0.0/' /etc/riak/riak.conf && \
    ...
    sed -i.bak 's/strong_consistency = off/strong_consistency = on/' /etc/riak/riak.conf && \
    sed -i.bak 's/storage_backend = bitcask/storage_backend = leveldb/' /etc/riak/riak.conf && \
